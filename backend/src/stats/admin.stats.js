...
const LoginEvent = require('./LoginEvent.model');
...
router.get('/visitors-today', async (req, res) => {
  try {
    ...
    const visitorsToday = await LoginEvent.distinct('user', { timestamp: { $gte: startOfDay } });
    res.json({ count: visitorsToday.length });
  } catch (error) {
    res.status(500).json({ message: 'Failed to fetch visitors today' });
  }
});
